...
export function flattenTrackStructure(trackStructure) {
    const files = {};
...
export function parseFieldTrack(fieldTrackStr, trackFieldName) {
  const result = {};
...
export async function createDirectoryStructure(obj, inputPath, rootPath) {
  try {
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (isDirectoryNode(value)) {
        ...
        await createDirectoryStructure(value, inputPath, currentPath);
      } else {
        await fs.mkdir(currentPath, { recursive: true });
...
export function isDirectoryNode(value) {
...
export function updateTrackField(trackField, newPath) {
